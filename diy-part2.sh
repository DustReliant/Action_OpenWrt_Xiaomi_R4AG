...
sed -i '/uci commit system/i\uci set system.@system[0].hostname='Wifi_R4AGV2'' package/lean/default-settings/files/zzz-default-settings

# 版本号里显示一个自己的名字（ababwnq build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i "s/OpenWrt /wbs build $(TZ=UTC-8 date "+%Y.%m.%d") @ Mr.Dust/g" package/lean/default-settings/files/zzz-default-settings
